        ...
        <div className="mb-4">
            ...
            <Field name={name}>{(props: FieldProps<any>) => (
                <div className="relative mb-1">
                    ...
                    <button data-cy={`${name}-dropdown-button`} className={`relative flex items-center justify-between rounded-lg mr-5 z-30 sm:mb-0 py-2 px-5 w-full ${props.meta.error && props.meta.touched ? "text-danger-red bg-danger-red bg-opacity-10 border-2 border-danger-red focus:outline-none focus:border-danger-red focus:ring-danger-red" : "border border-border-grey bg-white"} ${!props.field.value ? "text-lightgrey" : "text-darkgrey"}`} type="button" onClick={() => setIsOpen(!isOpen)}>
                    ...
                    {isOpen && <div data-cy={`${name}-dropdown-menu`} className="absolute bg-white rounded-lg shadow z-20 w-full py-2" tabIndex={-1}>
                        {options.map(option => (
