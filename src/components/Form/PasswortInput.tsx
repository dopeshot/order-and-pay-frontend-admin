            ...
            <Field type="password" name="password">{(props: FieldProps<any>) => (
                <>
                    <div className="relative flex flex-col justify-center my-1">
                        ...
                        <input data-cy={`password-input`} type={showPassword ? "text" : "password"} placeholder="Password" autoComplete="password" {...props.field} className={`font-roboto rounded-xl pl-4 py-2 ${props.meta.error && props.meta.touched ? 'bg-danger-red bg-opacity-10 border-2 border-danger-red focus:outline-none focus:border-danger-red focus:ring-danger-red' : 'border border-border-grey'}`} />
                    </div>
                </>
            )}</Field>
